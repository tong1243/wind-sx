...
public interface NavigationSettlementMapper extends BaseMapper<NavigationSettlement> {
    ...
    @Select("SELECT * FROM navigation_settlement WHERE navigation_session_id = #{navigationSessionId} LIMIT 1")
    NavigationSettlement selectByNavigationSessionId(@Param("navigationSessionId") String navigationSessionId);
}
